...
for n = 1:size(dateCell,2)
    ...
    for i = 1:maxRun
        ...
        if exist(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer1_3.mat'],'file') && exist(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer2_3.mat'],'file')
            for j = 1:3
                load(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer1_' num2str(j) '.mat']);
                ...
                title(['x baseline shift Layer 1 H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer1_' num2str(j) '.mat'])
                subplot(2,1,2)
...
%                     disp('No change')
                ...
                title(['y baseline shift Layer 1 H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer1_' num2str(j) '.mat'])
                subplot(2,1,2)
                plot(movementData.ballData(:,2))
                offsety = ginput(1);
                if offsety(1) < 0
...
%                     disp('No change')
                ...
                save(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer1_' num2str(j) '.mat'],'movementData');
            end
            for j = 1:3
                load(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer2_' num2str(j) '.mat']);
                figure(1);
                set(gcf, 'Position', get(0, 'Screensize'));
                subplot(2,1,1)
                plot(movementData.targetPosition(:,1))
                title(['x baseline shift Layer 2 H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer2_' num2str(j) '.mat'])
                subplot(2,1,2)
                plot(movementData.ballData(:,2))
                offsetx = ginput(1);
                if offsetx(1) < 0
...
%                     disp('No change')
                ...
                subplot(2,1,1)
                plot(movementData.targetPosition(:,2))
                title(['y baseline shift Layer 2 H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer2_' num2str(j) '.mat'])
                subplot(2,1,2)
                plot(movementData.ballData(:,2))
                offsety = ginput(1);
                if offsety(1) < 0
...
%                     disp('No change')
                ...
                movementData.targetPositionNoBaseline = movementData.targetPosition;
                movementData.targetPosition = [movementData.targetPosition(:,1) - offsetx(2), movementData.targetPosition(:,2) - offsety(2)];
                save(['H:/' dateCell{n}(1:2) '-' dateCell{n}(3:4) '-' dateCell{n}(5:6) '_MouseExp/' dateCell{n} '_0' runNumberStr '_processed_Layer2_' num2str(j) '.mat'],'movementData');
            end
        else
