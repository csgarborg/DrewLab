 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FUNCTION NAME:    plotMotionTracking3DRef
%
% FUNCTION:         plotMotionTracking3DRef(matFileName)
%
...
%     set(x2,'Position',[.05, .39, .9, .23])
    x3 = subplot(4,1,3);
    plot(1:size(movementData.targetPosition,1),medfilt1(movementData.targetPosition(:,3),medFiltSize),'g')
    xlabel('Frame')
    ylabel('Z Position (\mum)')
    grid on
    axis([1 size(movementData.targetPosition,1) floor(min(medfilt1([movementData.targetPosition(:,1);movementData.targetPosition(:,3)],medFiltSize))) ceil(max(medfilt1([movementData.targetPosition(:,1);movementData.targetPosition(:,3)],medFiltSize)))])
%     set(x3,'Position',[.05, .39, .9, .23])
    x4 = subplot(4,1,4);
    plot(movementData.ballData(:,1),movementData.ballData(:,2),'k')
    title('\fontsize{20pt}\bf{Ball Movement}')
    xlabel('Time (s)')
...
%     set(x4,'Position',[.05, .06, .9, .23])
    ...
    savefig(h,[matFileName(1:end-4) '_outputPlots_3DRef.fig']);
% else
%     load(matFileName1);
