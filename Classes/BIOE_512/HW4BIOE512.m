currentLevelVec = 1:5:1000;
fireRateVec = zeros(length(currentLevelVec));
for k = 1:length(currentLevelVec)
%===simulation time===
...
currentLevels = currentLevelVec(k); %Change this to see effect of different currents on voltage (Suggested values: 3, 20, 50, 1000)
...
[pks,locs,widths,prom] = findpeaks(V,t);
if length(locs) >= 2 && k > 2
fireRateVec(k) = 1/(mean(diff(locs))/1000);
end
close all
end

semilogx(currentLevelVec/1000,fireRateVec)
title('Action Potentials/s vs. Injected Current')
