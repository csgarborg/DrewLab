function [figHandle,ax1,ax2,ax3,ax4,ax5,ax6] = generateSleepScorePlotRL(procDataFileID,tStart,tEnd)
...
if isfield(ProcData,'EMG') && ...
    ...
    rightPower = zeros(refLen,1);
end

rightPower = rightPower(tMask);
...
if isfield(ProcData,'EMG') && ...
    ...
    leftPower = zeros(refLen,1);
end

leftPower = leftPower(tMask);

%% Right raw EMG
if isfield(ProcData,'EMG') && ...
        isfield(ProcData.EMG,'RightSignal')

    rightRaw = ProcData.EMG.RightSignal(:);

else
    rightRaw = zeros(refLen,1);
end

if numel(rightRaw) < refLen
    rightRaw(end+1:refLen) = rightRaw(end);
elseif numel(rightRaw) > refLen
    rightRaw = rightRaw(1:refLen);
end

rightRaw = rightRaw(tMask);

%% Left raw EMG
if isfield(ProcData,'EMG') && ...
        isfield(ProcData.EMG,'LeftSignal')

    leftRaw = ProcData.EMG.LeftSignal(:);

else
    leftRaw = zeros(refLen,1);
end

if numel(leftRaw) < refLen
    leftRaw(end+1:refLen) = leftRaw(end);
elseif numel(leftRaw) > refLen
    leftRaw = leftRaw(1:refLen);
end

leftRaw = leftRaw(tMask);
...
ax2 = subplot(7,1,2);
hold on
plot(tPlot,rightPower,'r')
if any(binForce)
    yMax = max(rightPower);
    ySpan = max(rightPower)-min(rightPower);
    if ySpan <= 0
        ySpan = 1;
    end
    scatter(tPlot(binForce),...
        yMax + 0.05*ySpan,...
        'filled')
end
ylabel('Right EMG')
...
ax3 = subplot(7,1,3);
hold on
plot(tPlot,leftPower,'b')
if any(binForce)
    yMax = max(leftPower);
    ySpan = max(leftPower)-min(leftPower);
    if ySpan <= 0
        ySpan = 1;
    end
    scatter(tPlot(binForce),...
